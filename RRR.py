...
def update_data(n):
    ...
    # Generate sample system stats (Replace with actual data)
    cpu_usage = random.randint(10, 90)
    memory_usage = random.randint(20, 80)
    temp = random.uniform(20.0, 50.0)

    # Event type distribution (Histogram)
    event_histogram = px.histogram(df, x="event_type", title="Event Type Distribution", color_discrete_sequence=['#FF6347'])
    event_histogram.update_layout(
    ...
    # Data table for latest detections
    table_data = df.tail(10).to_dict('records')  # Show latest 10 records

    # Update system stats
    cpu_stat = f"CPU: {cpu_usage}%"
    mem_stat = f"Memory: {memory_usage}%"
    temp_stat = f"Temperature: {temp:.2f}°C"

    return table_data, cpu_stat, mem_stat, temp_stat, event_histogram
