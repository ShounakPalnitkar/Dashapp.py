...
# Dash callback for updating battery graph
@dash_app.callback(
...
def update_battery(n):
    ...
    if df.empty:
        fig = px.line()
        fig.add_annotation(text="No battery data available", x=0.5, y=0.5, showarrow=False)
        return fig
...
@app.route('/')
def home():
    return render_template('control_panel.html')  # Requires templates/control_panel.html to exist
