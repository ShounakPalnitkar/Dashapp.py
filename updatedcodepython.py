...
def fetch_data():
    try:
        ...
        try:
            ...
            # Validate if the required keys exist in the response
            device_data = data.get("device_data", [])
            system_data = data.get("system_data", [])
            pi_data = data.get("pi_data", [])
            time_data = data.get("time_data", [])

            # If any of the key data is missing, print a warning and return empty DataFrames
            if not all([device_data, system_data, pi_data, time_data]):
                print("Warning: One or more expected data fields are missing.")
                return (
...
def update_data(n_intervals):
    ...
    # Create KPI Cards
    kpi_cards = [
        html.Div([html.H4(f"{cat}: {val}") for cat, val in system_df.itertuples(index=False, name=None)]
    ]

    # System Performance
    system_performance_figure = px.bar(system_df, x="Category", y="Value", title="System Performance")

    # Raspberry Pi Performance
    raspberry_pi_figure = px.line(pi_df, x="Category", y="Value", title="Raspberry Pi Performance")

    # Temperature Level
    temperature_level_figure = px.line(time_df, x="Time", y="Temperature", title="Temperature Level")

    # Object Detection Time
    object_detection_time_figure = px.line(time_df, x="Time", y="Detection Time", title="Object Detection Time")

    # Prepare Power Management Table
