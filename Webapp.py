...
app.layout = html.Div(style={'backgroundColor': 'black', 'color': 'white', 'padding': '20px'}, children=[
    ...
    # Interval component to refresh data every 1 second (adjust this for real-time streaming)
...
def update_data(n_intervals):
    ...
    if device_df.empty or system_df.empty or pi_df.empty or time_df.empty:
        ...
        return (
            ...
            px.pie(title="Raspberry Pi"),  # Placeholder for raspberry-pi
    ...
    # Pie chart for Raspberry Pi performance
    raspberry_pi_figure = go.Figure(go.Pie(labels=pi_df['Category'], values=pi_df['Value'], title="Raspberry Pi Performance"))
    raspberry_pi_figure.update_layout(title={'text': "Raspberry Pi Performance", 'x': 0.5, 'xanchor': 'center'})

    # Pie chart for System Performance
    system_performance_figure = go.Figure(go.Pie(labels=system_df['Category'], values=system_df['Value'], title="System Performance"))
    system_performance_figure.update_layout(title={'text': "System Performance", 'x': 0.5, 'xanchor': 'center'})
    ...
    # Return the updated figures and data
    return (
        px.bar(device_df, x="Device", y="Power Usage", title="Device Distribution").update_layout(title={'x': 0.5, 'xanchor': 'center'}),
        px.bar(system_df, x="Value", y="Category", orientation='h', title="Power Management").update_layout(title={'x': 0.5, 'xanchor': 'center'}),
        device_df.to_dict('records'),
        system_performance_figure,
        raspberry_pi_figure,
        px.line(time_df, x="Time", y="Detection Time", title="Temperature Levels").update_layout(title={'x': 0.5, 'xanchor': 'center'}),
        px.line(time_df, x="Time", y="Detection Time", title="Object Detection Time").update_layout(title={'x': 0.5, 'xanchor': 'center'}),
        kpi_cards
    )
